...
locals {
  ...
  cluster_role = try(aws_iam_role.eks-cluster-role[0].arn, var.iam_role_arn)
...
resource "aws_ec2_tag" "cluster_primary_security_group" {
  ...
  resource_id = aws_eks_cluster.eks-cluster[0].vpc_config[0].cluster_security_group_id
  key         = each.key
  value       = each.value
}
...
resource "aws_eks_addon" "eks-cluster-addons" {
  ...
  addon_version            = try(each.value.addon_version, data.aws_eks_addon_version.eks-cluster-addons[each.key].version)
...
data "aws_eks_addon_version" "eks-cluster-addons" {
  for_each = { for k, v in var.cluster_addons : k => v if local.create && !local.create_outposts_local_cluster }

  addon_name         = try(each.value.name, each.key)
  kubernetes_version = coalesce(var.cluster_version, aws_eks_cluster.eks-cluster[0].version)
...
resource "aws_eks_identity_provider_config" "this" {
  ...
  oidc {
    ...
    issuer_url                    = try(each.value.issuer_url, aws_eks_cluster.eks-cluster[0].identity[0].oidc[0].issuer)
