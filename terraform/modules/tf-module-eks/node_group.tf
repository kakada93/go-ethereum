...
resource "time_sleep" "this" {
  ...
  triggers = {
    ...
    cluster_endpoint = aws_eks_cluster.eks-cluster[0].endpoint
    cluster_version  = aws_eks_cluster.eks-cluster[0].version

    cluster_certificate_authority_data = aws_eks_cluster.eks-cluster[0].certificate_authority[0].data
  }
}
...
module "eks_managed_node_group" {
  ...
  cluster_primary_security_group_id = try(each.value.attach_cluster_primary_security_group, var.eks_managed_node_group_defaults.attach_cluster_primary_security_group, false) ? aws_eks_cluster.eks-cluster[0].vpc_config[0].cluster_security_group_id : null
